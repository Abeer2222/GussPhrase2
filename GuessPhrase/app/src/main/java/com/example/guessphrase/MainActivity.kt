...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sharedPreferences = this.getSharedPreferences(
            getString(R.string.score_file), Context.MODE_PRIVATE
        )
    ...
    private fun checkChar(guessC: Char) {
        ...
        if (found > 0) {
            theList.add("Found $found the letter ${guessC.toUpperCase()}")
        } else {
            theList.add("No $found the letter ${guessC.toUpperCase()}")
        }
        countG++
        val guessesLeft = 10 - countG
        if (countG < 10) {
            theList.add("$guessesLeft guesses remaining")
        }
        changeText()
